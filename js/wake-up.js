document.addEventListener("DOMContentLoaded", function () {
  ...
  function showEnding(event) {
    ...
    if (endings[selectedEnding]) {
      ...
      // 🔹 1.5초(1500ms) 후에 삼각형 보이기
      setTimeout(() => {
        next.style.display = "block";
        next.classList.add("blink");

        function goToWakeUpPage() {
          window.location.href = "wake-up.html";
        }

        // 🔹 모든 이벤트에서 이동 가능하도록 설정
        next.addEventListener("click", goToWakeUpPage);
        dialog.addEventListener("click", goToWakeUpPage);
